...
export default async function handleOrderPlaced({
  ...
  await manager.transaction(async (m) => {
    ...
    const orderActivity = logger.activity(
      `Splitting order ${data.id} into child orders...`
    );
    ...
    if (!order) {
      logger.failure(
        orderActivity,
        `OrderPlacedSubscriber | Order not found for order id ${data.id}.`
      );
      return;
    }
    ...
    for (const item of order.items) {
      ...
      if (!storeId) {
        logger.failure(
          orderActivity,
          `OrderPlacedSubscriber | product.store_id not found for product ${product.id}.`
        );
        continue;
      }
    ...
    for (const [storeId, items] of storesWithItems.entries()) {
      ...
      for (const shippingMethod of order.shipping_methods) {
        const shippingOption = await shippingOptionService.retrieve(
    ...
    logger.success(
      orderActivity,
      `Order ${data.id} has been split into ${storesWithItems.size} child orders.`
    );
  });
}
